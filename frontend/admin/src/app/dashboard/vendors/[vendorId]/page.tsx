        ...
        action === "approve"
          ...
          : action === "reject"
            ...
            : action === "suspend"
              ...
              : await adminApi.reactivateVendor(token, query.data.id, reason);
      void query.refetch();
      void updated;
      setReason("");
    } catch (caught) {
          ...
          <>
            ...
            query.data.status === "SUSPENDED" ? (
              <Card>
                ...
                {query.data.status === "APPROVED" ? (
                  <p className="mt-2 text-sm text-slate-600">
                    Suspending immediately removes this vendor from public
                    discovery. The vendor receives an in-app notification and,
                    when email is configured, an email explaining the review.
                  </p>
                ) : null}
                {query.data.status === "SUSPENDED" ? (
                  <p className="mt-2 text-sm text-slate-600">
                    This profile is hidden from public discovery. Reactivate it
                    when the vendor is cleared to return.
                  </p>
                ) : null}
                <label
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  {query.data.status === "SUSPENDED" ? (
                    <Button
                      disabled={busy}
                      type="button"
                      onClick={() => void decide("reactivate")}
                    >
                      Reactivate vendor
                    </Button>
                  ) : null}
