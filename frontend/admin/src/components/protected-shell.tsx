...
const administrationNavigation = [
  ["/dashboard/admins", "Admin access", "01"],
  ["/dashboard/security", "Account security", "02"],
  ["/dashboard/catalogue", "Categories & cities", "03"],
] as const;
  ...
  useEffect(() => {
    ...
    adminApi
      ...
      .then((result) => {
        ...
        setChecking(false);
        return adminApi
          .verificationQueue(accessToken, "pageSize=1")
          .then((queue) => setPendingVendors(queue.total))
          .catch(() => setPendingVendors(null));
      })
      ...
      <div className="setu-admin-layout">
        <nav aria-label="Operations navigation" className="setu-admin-sidebar">
          ...
          <div className="setu-admin-nav-group">
            ...
            {primaryNavigation.map(([href, label, index]) => (
              <Link
                ...
                key={href}
              >
                <span aria-hidden="true" className="setu-admin-nav-index">
                  {index}
                </span>
                <span className="setu-admin-nav-label">{label}</span>
                {href === "/dashboard/vendors" && pendingVendors ? (
                  <b className="setu-admin-nav-count">{pendingVendors}</b>
                ) : null}
              </Link>
            ))}
          </div>
          {admin?.role !== "REVIEWER" ? (
            <div className="setu-admin-nav-group">
              <p>Administration</p>
              {administrationNavigation.map(([href, label, index]) => (
                <Link
                  className={`setu-admin-nav-link ${isActive(href) ? "setu-admin-nav-link-active" : ""}`}
                  href={href}
                  key={href}
