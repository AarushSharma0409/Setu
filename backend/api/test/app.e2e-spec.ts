...
class FakePrisma {
  ...
  user = {
    ...
    findFirst: async ({
      ...
      where: {
        ...
        OR?: Array<{ email?: string | undefined; phone?: string | undefined }>;
      };
      select?: { id: true };
    }) => {
      const alternatives = where.OR ?? [where];
      const user = [...this.users.values()].find((candidate) =>
        alternatives.some(
          (alternative) =>
            (alternative.email !== undefined &&
              candidate.email === alternative.email) ||
            (alternative.phone !== undefined &&
              candidate.phone === alternative.phone),
        ),
      );
      return user ? (select ? { id: user.id } : user) : null;
    },
    update: async ({
      where,
      data,
  ...
  notification = {
    create: async ({ data }: { data: NotificationRecord }) => {
...
const testEnv = {
  values: {
    ...
    CLAMAV_PORT: 3310,
    DOCUMENT_SCAN_TIMEOUT_MS: 30_000,
    ...
    INSURANCE_PRODUCT_CATALOG_ENABLED: false,
    INSURANCE_CUSTOMER_NEEDS_ENABLED: false,
    INSURANCE_QUOTATION_ENABLED: false,
    ...
    INSURANCE_PRODUCTION_APPROVED: false,
    INSURANCE_PROVIDER_DEFAULT_TIMEOUT_MS: 8_000,
    INSURANCE_PROVIDER_MAX_RETRIES: 1,
    INSURANCE_PROVIDER_ALLOWED_HOSTS: [],
    INSURANCE_HANDOFF_TTL_MINUTES: 15,
    INSURANCE_WEBHOOK_MAX_BODY_BYTES: 65_536,
    ...
    INSURANCE_NEED_ASSESSMENT_DRAFT_TTL_DAYS: 30,
    INSURANCE_MAX_COVERED_MEMBERS: 6,
    INSURANCE_SENSITIVE_DATA_ENCRYPTION_KEY:
      "MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=",
    INSURANCE_DOCUMENT_MAX_FILE_SIZE_BYTES: 10 * 1024 * 1024,
    INSURANCE_DOCUMENT_ALLOWED_MIME_TYPES: [
      "application/pdf",
      "image/jpeg",
      "image/png",
    ],
...
describe("Setu API foundation", () => {
  ...
  it("allows normal customer registration in production mode", async () => {
    ({ app } = await createApp({
      ...testEnv,
      isProduction: true,
    ...
    await request(app.getHttpServer())
      ...
      .send({
        ...
        email: "production.user@setu.test",
  ...
  it("supports vendor onboarding through submission", async () => {
    ...
    expect(setup.fakePrisma.notifications).toEqual([
