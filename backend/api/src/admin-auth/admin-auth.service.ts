...
export class AdminAuthService {
  ...
  async login(email: string, password: string) {
    ...
    return this.beginSecondFactorChallenge(
      admin,
      "ADMIN_LOGIN_PASSWORD_SUCCEEDED",
    );
  }

  private async beginSecondFactorChallenge(
    admin: {
      id: string;
  ...
  async security(adminId: string) {
    ...
    return {
      admin: { ...adminView(admin), lastLoginAt: admin.lastLoginAt },
      activeSessions,
      recoveryCodesRemaining: await this.prisma.adminRecoveryCode.count({ where: { adminUserId: adminId, usedAt: null } }),
    };
  }

  async changePassword(adminId: string, currentPassword: string, newPassword: string) {
    const admin = await this.requireActiveAdmin(adminId);
    if (!(await this.passwordService.verify(currentPassword, admin.passwordHash))) {
      throw new UnauthorizedException("Current password is incorrect");
    }
    await this.prisma.$transaction(async (tx) => {
      await tx.adminUser.update({ where: { id: adminId }, data: { passwordHash: await this.passwordService.hash(newPassword), failedLoginCount: 0, lockedUntil: null } });
      await this.sessionService.revokeAllForAdmin(adminId, tx);
      await tx.auditLog.create({ data: { adminUserId: adminId, action: "ADMIN_PASSWORD_CHANGED", entityType: "AdminUser", entityId: adminId } });
    });
    return { ok: true };
  }
  ...
  async regenerateRecoveryCodes(adminId: string, code: string) {
    const admin = await this.requireActiveAdmin(adminId);
    if (!admin.twoFactorSecretEncrypted || !this.totpService.verify(code, this.encryptionService.decrypt(admin.twoFactorSecretEncrypted))) {
      throw new UnauthorizedException("Invalid two-factor code");
    }
    const recoveryCodes = createRecoveryCodes();
    const records = await Promise.all(recoveryCodes.map(async (recoveryCode) => ({ adminUserId: adminId, codeHash: await bcrypt.hash(recoveryCode, 12) })));
    await this.prisma.$transaction(async (tx) => {
      await tx.adminRecoveryCode.deleteMany({ where: { adminUserId: adminId } });
      await tx.adminRecoveryCode.createMany({ data: records });
      await tx.auditLog.create({ data: { adminUserId: adminId, action: "ADMIN_RECOVERY_CODES_REGENERATED", entityType: "AdminUser", entityId: adminId } });
    });
    return { recoveryCodes };
  }

  async revokeAllSessions(adminId: string) {
    await this.sessionService.revokeAllForAdmin(adminId);
    await this.auditService.record({ adminUserId: adminId, action: "ADMIN_ALL_SESSIONS_REVOKED", entityType: "AdminUser", entityId: adminId });
    return { ok: true };
  }
