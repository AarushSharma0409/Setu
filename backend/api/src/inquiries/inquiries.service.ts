...
export class InquiriesService {
  ...
  async create(
    ...
    try {
      ...
      const emailContext = await this.prisma.inquiry.findUnique({
        where: { id: created.id },
        select: {
          referenceNumber: true,
          subject: true,
      ...
      if (emailContext) {
        await this.mail.send({
          event: "inquiry_customer_acknowledgement",
          to: emailContext.user.email,
          subject: "Your Setu inquiry has been sent",
          title: "Your inquiry is on its way",
          body: `Hello ${emailContext.user.name ?? "there"},\n\nYour inquiry ${emailContext.referenceNumber} has been sent to ${emailContext.vendor.businessName ?? "the provider"}. You can follow replies securely in Setu.`,
          cta: {
            label: "View inquiry",
            url: this.mail.publicUrl(`/account/inquiries/${created.id}`),
          },
        });
        await this.mail.send({
          event: "inquiry_vendor_notification",
          to: emailContext.vendor.contactEmail ?? emailContext.vendor.owner.email,
          subject: "You received a new inquiry on Setu",
          title: "A customer sent a new inquiry",
          body: `Inquiry: ${emailContext.referenceNumber}\nSubject: ${emailContext.subject}\n\nOpen your vendor workspace to view the full request and reply securely.`,
          cta: {
            label: "View inquiry",
            url: this.mail.publicUrl(`/vendor/inquiries/${created.id}`),
          },
        });
      }
  ...
  async vendorStatus(
    ...
    if (isCustomerNotifiableStatus(toStatus)) {
      await this.mail.send({
        event: "inquiry_status_updated",
        to: current.user.email,
        subject: "Update on your Setu inquiry",
        title: "Your inquiry status changed",
        body: `Hello ${current.user.name ?? "there"},\n\nYour inquiry ${current.referenceNumber} with ${current.vendor.businessName ?? "the provider"} is now ${formatInquiryStatus(toStatus)}. ${inquiryStatusMessage(toStatus)}`,
        cta: {
          label: "View inquiry",
          url: this.mail.publicUrl(`/account/inquiries/${current.id}`),
        },
      });
    }
  ...
  private async userTransition(
    ...
    if (toStatus === InquiryStatus.WITHDRAWN) {
      await this.mail.send({
        event: "inquiry_withdrawn",
        to: current.vendor.contactEmail ?? current.vendor.owner.email,
        subject: "A Setu inquiry has been withdrawn",
        title: "An inquiry was withdrawn",
        body: `Inquiry ${current.referenceNumber} has been withdrawn by the customer. No further action is needed.`,
        cta: {
          label: "Open vendor workspace",
          url: this.mail.publicUrl(`/vendor/inquiries/${current.id}`),
        },
      });
    }
...
function isCustomerNotifiableStatus(status: InquiryStatus): boolean {
  const customerNotifiableStatuses: InquiryStatus[] = [
    InquiryStatus.CONTACTED,
    InquiryStatus.IN_PROGRESS,
    InquiryStatus.RESOLVED,
    InquiryStatus.CLOSED,
  ];
  return customerNotifiableStatuses.includes(status);
}

function formatInquiryStatus(status: InquiryStatus): string {
  return status.toLowerCase().replaceAll("_", " ");
}

function inquiryStatusMessage(status: InquiryStatus): string {
  const messages: Partial<Record<InquiryStatus, string>> = {
    [InquiryStatus.CONTACTED]: "The provider has marked your inquiry as contacted.",
    [InquiryStatus.IN_PROGRESS]: "The provider is working on the request.",
    [InquiryStatus.RESOLVED]: "The provider has marked the request as resolved.",
    [InquiryStatus.CLOSED]: "This inquiry is now closed.",
  };
  return messages[status] ?? "Open Setu to view the latest details.";
}
